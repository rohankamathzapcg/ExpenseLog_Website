                    ...
                    {categories && (
                        <div className="row border rounded shadow-sm border">
                            <div className="col p-3">
                                <span className='text-start headings-side'>Standard Categories</span>
                                <div className='mt-3'>
                                    {categories.map((category, index) => (
                                        <span
                                            key={index}
                                            className={`badge mt-3 p-2 me-2 ${isSelected(category.categoryName) ? 'background-badge-blocked text-blocked' : 'background-badge text-badge'}`}
                            ...
                            <div className="col p-3">
                                ...
                                <div className='mt-3'>
                                    {myCategory.map((category, index) => (
                                        <span key={index} className='background-badge-selected badge mt-3 p-2 me-2'>{category.catName}</span>
                                    ))}
                                </div>
                                <div className="d-flex justify-content-end mt-3">
                                    <button type="button" style={{ border: "1px solid #012970", backgroundColor: 'white', color: '#012970', fontFamily: '"Merriweather", sans-serif', fontSize: "12px" }} className="btn button-Save" onClick={handleSaveMyCategory}>Save changes</button>
                                </div>
                            </div>
                        </div>
                    )}
