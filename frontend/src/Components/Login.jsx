...
const Login = () => {
    ...
    const fetchGoogleLoginResponse = () => {
        axios.get('https://localhost:7026/api/SocialAuth/google-response')
            .then((result) => {
                if (result.status === 200) {
                    ...
                    // Store the auth data in session storage and set it in state
                    window.sessionStorage.setItem('authUser', JSON.stringify(result.data));
                    setAuthUser(result.data); // Set the state with the parsed data
                    setIsLoggedIn(true);
    ...
    //     // Listen for the response from the redirect (after the user completes Google login)
    //     window.addEventListener('message', (event) => {
    //         if (event.origin !== "https://localhost:7026") {
    //             return;
    //         }
                        ...
                        <div className="modal-body signin_modal" style={{ padding: 0 }}>
                            ...
                            <div className="social_login">
                                ...
                                <ul className="social_log">
                                    <li onClick={redirectToGoogleLogin}><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 48 48" className="mb-1 me-2">
